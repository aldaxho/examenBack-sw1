...
class _CategoriaListScreenState extends State<CategoriaListScreen> {
  ...
  Future<void> _loadItems() async {
    ...
    try {
      final items = await CategoriaService.getAll();
      setState(() {
        _items = items;
        _isLoading = false;
      });
  ...
  Future<void> _deleteItem(Categoria item) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Confirmar eliminación'),
        content: Text('¿Estás seguro de que quieres eliminar este categoria?'),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      try {
        await CategoriaService.delete(item.id!);
        _loadItems();
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Categoria eliminado correctamente')),
        );
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('No hay categoria registrados'),
          ],
        ),
      );
    }
          ...
          return Padding(
            ...
            child: InkWell(
              onTap: () => Navigator.push(
                context,
                PageRouteBuilder(
                  pageBuilder: (_, __, ___) => CategoriaDetailScreen(item: item),
                          ...
                          switch (value) {
                            case 'view':
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => CategoriaDetailScreen(item: item),
                                ),
                              );
                              break;
                            case 'edit':
